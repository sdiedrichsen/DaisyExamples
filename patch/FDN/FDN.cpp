...
static float __attribute__((section(".dtcmram_bss")))
fdnMatrix[numDelays][numDelays];
...
void AudioCallback(float** in, float** out, size_t size)
{
    ...
    for(size_t i = 0; i < size; i++)
    {
        ...
        out[0][i]
            = input
              + wet * (delayOut[0] - delayOut[2] + delayOut[4] - delayOut[6]);
        out[1][i]
            = input
              + wet * (-delayOut[1] + delayOut[3] - delayOut[5] + delayOut[7]);
    }
}
...
int main(void)
{
    ...
    ctrl[eHiCut].Init(
        "High Cut", "Hz", 1e3f, 2e4f, 100, eLogarithmic, 12e3f, 0);
    ...
    MenuCtrl.Init(
        "Selection", "", 0.f, eNumControls - 1, eNumControls, eLinear, 0.f, 0);
